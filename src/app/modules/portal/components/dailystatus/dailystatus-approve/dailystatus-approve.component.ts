...
@Component({
  selector: 'app-dailystatus-approve',
  templateUrl: './dailystatus-approve.component.html',
  styleUrls: ['./dailystatus-approve.component.css']
})
export class DailystatusApproveComponent implements OnInit {
          ...
          for (const id of this.listOfEmployees) {
            ...
            for (let j = 0; j < res.data.length; j++) {
              if (id == res.data[j].id) { 
	              ...
	              if(res.data[j].status == 'Pending'){
		               var str = new Date(res.data[j].date).toDateString().split(" ");
	                   date = date +str[2]+" "+str[1] + " , ";
                    }
            ...
            if(date != '')
            this.list.push({ empId: id, name: name, submittedDate: date.replace(/,\s*$/, ""), status: 'Pending' });
            else
             this.list.push({ empId: id, name: name, submittedDate: '', status: 'Approved' });
          }
